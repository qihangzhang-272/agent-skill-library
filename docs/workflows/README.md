...
稳定写作发布链在 `plugins/orchestrator/skills/workflow-orchestrator/references/chains/wechat-writing.md`：按需采集账号级公开语料 -> agent-reach 搜索沉淀 -> `public-account-writing-style` 完成编辑判断、深度写作、事实审校与 AI 味终审 -> Markdown 排版 -> 由 `editorial-visual-storytelling` 独立生成视觉计划 -> 按计划调用截图复用 / 通用插图 / 信息图 / SVG 图解 / 知识漫画与封面 -> `qihang-wechat-layout` 生成正式预览和个人样式工作台，并统一发布排版 -> 以最终 Markdown 推送公众号草稿箱。账号语料为可选；Chain 只规定顺序、交接和落盘，不承担领域判断。
