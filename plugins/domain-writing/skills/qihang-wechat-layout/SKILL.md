...
description: Apply the confirmed “航的杂谈地图” personal typography catalog to finalized Markdown and generate WeChat-compatible HTML using the same renderer as draft publishing. Use when the user asks for 启航排版、个人公众号风格、个人样式目录、HTML 预览 or wants to replace generic Baoyu themes. Do not use it to rewrite content or decide illustrations.
---

# 启航公众号排版

把已经定稿的 Markdown 转成“航的杂谈地图”统一排版。个人样式目录是配置，不再生成独立效果工作台。
...
3. 在普通 HTML 里检查 677px 与 375px 阅读宽度、断图、层级、引用、列表、表格和横向溢出。
4. 发布时把同一 Markdown 交给 `baoyu-post-to-wechat`。预览和发布均调用 `scripts/qihang-layout.ts`，不得各维护一份 CSS。
5. 若进入草稿箱，在发布回执中记录主题、主色、图片数和草稿结果。

## Personal Style Rule

- `assets/qihang-style-catalog.json` 是个人组件样式与默认值真源；只服务正式渲染，不承担预览交互。
...
- 产生：微信兼容 HTML；完整发布任务中可继续产生草稿箱回执。
- 可接上游：任何能交付定稿 Markdown 的写作或组装技能。
- 可接下游：人工浏览检查或 `baoyu-post-to-wechat`；两者互不强制。
