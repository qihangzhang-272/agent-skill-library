...
| 股票行情/雪球/热股 | finance | [references/finance.md](references/finance.md) |
...
- [股票行情](references/finance.md) - 雪球（A股/港股/美股行情、搜索、热帖、热股）
...
- 按平台路由读取对应 reference，先检查可用后端，完成用户明确要求的公开信息获取范围。
- 返回来源、时间、覆盖范围、失败与限制；不得把登录失败、样本不足或搜索摘要冒充完整事实。
- 只执行只读获取；发布、点赞、评论、私信、私人数据读取或安装更新必须另行获得授权。
