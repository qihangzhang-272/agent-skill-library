...
- product-analysis -> 产品+投资视角 -> 研报 -> 可视化。分析和呈现不做成平行分类。
- 技术开发范式和项目初始化。
- 写作、改写和内容定稿。
- 运营、同步和治理。
...
- product-analysis 这类链路应保持：产品判断和投资视角先由 `ai-product-analyzer` 或 `oss-investment-scorecard` 产出，再进入研报，最后可视化。产品投资分析研报默认走完整链路，除非用户明确只要 IC memo、DD 文本或前端 brief。
- 可视化研报页的体验原则：现代风、全展开、不堆叠、不把核心内容藏进折叠组件；页面结构沿用上游产品/投资研报结构，不由 orchestrator 重新发明论证顺序。
