...
- 写作发布：把用户对话与已有材料作为入口，按当前缺口动态调用研究、写作、视觉、排版与发布瓦技能；不预设从选题到发布的完整路线。
...
瓦技能可独立存在。技能图是运行时能力地图：每轮只选择输入已满足、能直接推进当前目标的真实技能；chain 只用于顺序确实不可交换的任务。当技能进入编排时，要能解释它提供和消费的材料：

- 公众号可从对话、想法、资料、草稿、图片或定稿任意进入；研究、写作、视觉和排版只在当前缺口需要时调用，发布前预览与授权是唯一固定尾部闸门。
