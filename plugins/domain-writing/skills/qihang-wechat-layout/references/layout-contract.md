...
`scripts/build-publish-preview.ts` 只把已经生成的正式 HTML 包装为 677px 与 375px 双宽度质检页。它不编译第二套样式，也不能修改个人样式目录。
...
7. 发布任务生成 `06-final.preview.html`，其中没有主题、字号、间距或组件切换控件。
8. 预览发现断图、主题缺失或横向溢出时，停止发布并回到排版或资产 owner 修复。
