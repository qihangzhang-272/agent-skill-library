# Section Docs

这里放各板块自己的来源、调用链、索引和收录结果。板块入口仍然在 `sections/<section>/README.md`，这里保存更细的文档。

## 当前板块

| 板块 | 文档路径 | 当前内容 |
| --- | --- | --- |
| frontend-design | `docs/sections/frontend-design/` | 来源清单、网页 prompt 索引、前端 UI 调用链 |

## 未来板块

后续可按同样结构扩展：

- `docs/sections/investment/`
- `docs/sections/product/`
- `docs/sections/technology/`
- `docs/sections/development-paradigms/`
- `docs/sections/operations/`

## 放置标准

如果文档只服务一个板块，放在对应 `docs/sections/<section>/` 下。如果它定义整个仓库的规则，放到 `docs/governance/`；如果它是跨板块可复用流程，放到 `docs/workflows/`。
