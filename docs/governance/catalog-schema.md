...
- `wechat-account-corpus-research`、`topic-research-deposition` 与 `public-account-writing-style` 是本地维护的写作链路瓦技能，进入 `domain-writing` plugin；正文 owner 是 `public-account-writing-style`，账号 corpus 只在用户要求历史或对标账号时按需调用。外部写作技能只在 skill index 记录来源，不复制进本库。
