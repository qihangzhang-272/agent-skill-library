...
export function buildQihangStudio(options: BuildStudioOptions): string {
  ...
  const typeTargetOptions = catalog.typography.targets
    ...
    .join("");
  const componentOptions = renderOptions(catalog.components);
...
<body>
  <main class="studio-shell">
    <header class="studio-masthead">
      <div class="studio-brand"><span class="studio-brand-mark">航</span><div><strong>航的杂谈地图 · 排版工作台</strong><small>QIHANG ARTICLE STUDIO / PERSONAL STYLE SET</small></div></div>
      <div class="studio-status">实时正文 · 个人样式集 · 单文件预览<br>默认发布样式与备选效果并列可见</div>
    </header>
    <section class="studio-workspace">
      ...
      <aside class="control-deck">
        <div class="control-heading"><div><h2>排版设置</h2><p class="control-note">这里只保留已经确认的个人样式。</p></div><span>SELECTED</span></div>
    ...
    <section class="gallery-section" data-studio-gallery="selected-variants">
      <div class="gallery-head"><div><h2>启航个人样式集</h2><p>${totalStudioVariants(catalog)} 种已确认效果同时陈列。主题、文字和间距控制在上方工作台实时查看。</p></div><p>每组第一个样式是正式发布默认值；“或”后面的样式保留为可切换备选。</p></div>
  ...
  <script>
    ...
    const selectorMap={h1:'h1',h2:'h2',h3:'h3',h4:'h4',h5:'h5',h6:'h6',body:'p',quote:'blockquote',strong:'strong','ordered-list':'ol li','unordered-list':'ul li',table:'table'};
    ...
    const variantsFor=(target)=>catalog.components.find(component=>component.id===target).options;
    const refreshVariants=()=>{const target=componentTarget.value;componentVariant.innerHTML=variantsFor(target).map(item=>'<option value="'+item.id+'">'+item.label+'</option>').join('');componentVariant.value=catalog.defaults[target]||componentVariant.options[0]?.value||''};
