...
- `wechat-writing.md`：可选账号语料采集、主题研究、`human-writing` 写作与改稿、领域视觉计划、按计划生产、HTML 预览与草稿箱发布。

稳定写作发布链在 `plugins/orchestrator/skills/workflow-orchestrator/references/chains/wechat-writing.md`：按需采集账号级公开语料 -> agent-reach 搜索沉淀 -> `human-writing` 写作与改稿 -> Markdown 排版 -> 由 `editorial-visual-storytelling` 独立生成视觉计划 -> 按计划调用截图复用 / 通用插图 / 信息图 / SVG 图解 / 知识漫画与封面 -> HTML 预览 -> 以最终 Markdown 推送公众号草稿箱。账号语料只在用户要求历史或对标账号时运行；Chain 只规定顺序、交接和落盘，不承担研究、写作规则、视觉内容与形式判断。
