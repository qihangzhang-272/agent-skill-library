...
**外部完整技能迁移**：如果用户明确要求"直接迁移"某个外部技能，先确认它是候选收藏还是要晋升为本地可调用技能包。候选收藏先进入 `qihang-skill-index`；明确晋升后才允许放进 `plugins/*/skills/`。新增本地技能包时必须记录上游仓库 URL、上游 commit、许可说明、是否替代了本地旧技能——这些字段填在 `qihang-skill-index` 登记表的 Notes 列（5 字段登记法见 `plugins/skill-index/skills/qihang-skill-index/references/registration-guide.md`），不再用独立 yaml 块记录。如果外部技能被吸收到启航工作包，只保留为 `references/`，不要再把旧技能作为平行 runtime 入口分发。
